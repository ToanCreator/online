...
document.addEventListener('DOMContentLoaded', function() {
    // Danh sách ảnh mẫu
    const presetImages = [
        "https://i.ibb.co/3yRrFnC5/Live-Wallpaper-Makima-Chainsawman-46116.gif",
...
const subjectBackgrounds = [
    "https://i.ibb.co/3yRrFnC5/Live-Wallpaper-Makima-Chainsawman-46116.gif", // Toán
...
// ==================== HỆ THỐNG BÌNH LUẬN ====================
// Biến toàn cục
const ADMIN_EMAIL = "tranhoangtoan2k8@gmail.com";
...
function initAuthState() {
    auth.onAuthStateChanged((user) => {
        if (user) {
            ...
            if (isAdmin) {
                userInfoGroup.innerHTML = `
                    <label>
                        <h3 style="color: white; font-weight: bold; text-shadow: -1px -1px 0 #000, 1px -1px 0 #000, -1px 1px 0 #000, 1px 1px 0 #000;">
                            Tên: <span class="admin-badge">ADMIN</span>
                        </h3>
                    </label>
                    <input type="text" id="username" value="${user.displayName || 'Admin'}" readonly>
                `;
            } else {
                userInfoGroup.innerHTML = `
                    <label for="username">
                    ...
                    <input type="text" id="username" value="${user.displayName || ''}" required>
                `;
            }
        } else {
            // Chưa đăng nhập
...
function updateWordCount() {
    ...
    wordCountSpan.style.color = wordCount > 200 ? '#e74c3c' : '#ddd';
}
...
async function addComment(e) {
    ...
    try {
        // Nếu là admin hoặc người dùng đã đăng nhập
        if (isAdmin) {
            username = "ADMIN ✅";
        } else if (user) {
...
function updateCommentsDisplay() {
    ...
    if (commentsToShow.length === 0) {
        commentsList.innerHTML = '<p class="no-comments">Chưa có bình luận nào. Hãy là người đầu tiên!</p>';
    ...
    commentsToShow.forEach(comment => {
        ...
        if (currentUser && (currentUser.email === ADMIN_EMAIL || currentUser.uid === comment.userId)) {
